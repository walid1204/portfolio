...
$(function () {
    ...
    const modalVideoContainer = $('.video-container');

    $('.portfolio-trigger, .portfolio-trigger-title').on('click', function (e) {
        e.preventDefault();

        const trigger = $(this).hasClass('portfolio-trigger') ? $(this) : $(this).closest('.portfolio-item-inner').find('.portfolio-trigger');
        const dataSource = $(this).hasClass('portfolio-trigger') ? $(this) : $(this).closest('.portfolio-item').find('.portfolio-trigger');

        const title = dataSource.data('title');
        const description = dataSource.data('description'); // This can now be HTML
        let videoSrc = dataSource.data('video-src');

        modalTitle.text(title);
        modalDescription.html(description); // Use .html() to render tags

        // Video Logic
        modalVideoContainer.empty(); // Clear previous content

        if (videoSrc) {
            modal.removeClass('no-video');
            if (videoSrc.includes('drive.google.com')) {
                // Convert view/sharing links to preview links for embedding
                videoSrc = videoSrc.replace('/view?usp=sharing', '/preview').replace('/view', '/preview');
                ...
                modalVideoContainer.append(iframe);
            } else {
                // Standard MP4 video
                ...
                videoTag.append('Your browser does not support the video tag.');
                modalVideoContainer.append(videoTag);
            }
        } else {
            modal.addClass('no-video');
        }
    ...
    $('.close-modal').on('click', function () {
        ...
        modalVideoContainer.empty(); // Stop video/iframe
    });
